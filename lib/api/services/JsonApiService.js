...
module.exports = {
  ...
  init: function() {
    for (let name in sails.models) {
      ...
      model.associations.forEach(association => {
        ...
        sails.log.verbose(
          name + " has a relation with " + type + " (as " + alias + ")"
        );
        associations[alias] = {
          type: pluralize(type)
        };
    ...
    if (_.isArray(data) || _.isPlainObject(data)) {
      converted = _.transform(data, (result, value, key) => {
        if (_.isArray(value) || _.isPlainObject(value)) {
          result[
            this._convertCase(key, convertCaseOptions)
          ] = this._convertCase(value, convertCaseOptions);
        } else {
  ...
  /**
   * uniformizeQueryParams:
   * sadly, when sails "transforms" a socket request into a REST one, it doesn't treat the complex params (e.g. ?filter[name]=something) like it does for the normal requests ...
   * Important NOTE: this does not handle params like a[b]=stuff&a[b]=otherstuff
   * @param {Object} req request object from sails
   * @return {Object} the same request
   */
  uniformizeQueryParams: function(req) {
    const regex = /(.*)\[(.*)\]/;
    const uniformizedQuery = Object.entries(req.query).reduce(
      (acc, [key, value]) => {
        const matches = regex.exec(key);
        if (matches) {
          let [_, newKey, nestedKey] = matches;
          return {
            ...acc,
            [newKey]: {
  ...
  serialize: function(modelName, data) {
    ...
    if (
      (_.isArray(data) && depthOf(data) > 2) ||
      (_.isObjectLike(data) && _.isArray(data) === false && depthOf(data) > 1)
    ) {
      returnedValue = Serializer.serialize(modelName, data);
    } else {
      returnedValue = Serializer.serialize(
        modelName,
        data,
        "noRelationships-" + modelName
      );
    }
  ...
  serializeWaterlineValidationError: function(data) {
    ...
    var targetAttributes =
      data.Errors !== undefined ? data.Errors : data.invalidAttributes;
        ...
        errors.push({
          ...
          source: {
            pointer:
              ...
              this._convertCase(
                attributeName,
                this.getAttributesSerializedCaseSetting()
              )
          }
        });
      }
